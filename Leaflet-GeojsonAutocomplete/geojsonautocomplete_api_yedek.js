...
(function ($) {
    ...
    var options = {
        geojsonServiceAddress: "http://localhost:8080/CitysurfLeafletAdmin/FullTextSearchServlet",
        placeholderMessage: "Ne Arıyorsunuz?",
        searchButtonTitle: "Arama",
        clearButtonTitle: "Temizle",
        foundRecordsMessage: "arası kayıtlar gösteriliyor."
    };
    $.fn.GeoJsonAutocomplete = function () {
        $(this).each(function () {
            var element = $(this);
            element.addClass("searchContainer");
            element.append('<input id="searchBox" class="searchBox" placeholder="' + options.placeholderMessage + '"/>');
            element.append('<input class="searchButton" onclick="searchButtonClick()" type="submit" value="" style="vertical-align: middle;"  title="' + options.searchButtonTitle + '"/>');
            element.append('<span style="width: 24px;border-left: solid 1px lightgrey;margin-left: 10px;"></span>');
            element.append('<input class="clearButton" onclick="clearButtonClick()" type="submit"  value="" style="vertical-align: middle;"  title="' + options.clearButtonTitle + '">');
